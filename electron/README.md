...
安装包输出：`dist\desktop\DeepWiki-Setup-<version>.exe`。这是自包含安装包，目标电脑**不需要** Python 3、Node、Git、Docker、Visual Studio 或外网。仅有 Python 2.7 的 64 位 Windows 10+ 也可以直接安装运行。脚本会：

1. `next build`（standalone）
2. 下载便携 Python 3.11、官方 Node、MinGit，并附带 VC++ 运行库 DLL
3. 安装 API 依赖，预下载本地嵌入模型，拷贝后端与前端到 `electron/resources`
4. 用 electron-builder 生成 NSIS 安装包

离线电脑仍需要一个可访问的 OpenAI 兼容对话接口（内网网关或本机 Ollama）。远程 Git 仓库克隆需要网络；本地文件夹仓库可以完全离线分析。
