const STRINGS = {
  zh: {
    ...
    setupTitle: '连接模型',
    setupHint:
      '填写兼容 OpenAI 的 Base URL、密钥和对话模型。Wiki 索引会在本机直接运行 Embedding 模型，不需要 Ollama，也不需要网关提供 /embeddings。',
    baseUrl: 'Base URL',
    apiKey: 'API Key',
    model: 'Model',
  ...
  en: {
    ...
    setupHint:
      'Enter an OpenAI-compatible Base URL, API key, and chat model. Wiki indexing runs a local embedding model in-process — no Ollama and no POST /embeddings on the chat gateway.',
    baseUrl: 'Base URL',
    apiKey: 'API Key',
    model: 'Model',
